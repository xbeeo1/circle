...
class SaleOrderLineInheritLs(models.Model):
    ...
    @api.depends('tax_ids', 'price_unit', 'product_uom_qty','discount_amount')
    def _compute_gst_amount(self):
        for line in self:
            gst_rate = 0.0
            tax_id= line.tax_ids
            if tax_id:
                gst_rate = sum(tax.amount for tax in tax_id) / 100
            line.gst_amount = (line.price_unit * line.product_uom_qty - line.discount_amount ) * gst_rate
    ...
    def _prepare_invoice_line(self, **optional_values):
        res = super(SaleOrderLineInheritLs, self)._prepare_invoice_line()
