...
class PartnerPurchaseHistory(models.Model):
    _name = 'partner.purchase.history'
    ...
    sequence = fields.Integer(string="Sequence", help="Gives the sequence order when displaying a list of purchase quote lines.", default=10)

    product_id = fields.Many2one(comodel_name='product.product', string="Product",  check_company=True)
    ...
    taxes_id = fields.Many2many('account.tax', string="Taxes", store=True, context={'active_test': False},  check_company=True)
...
class TotalPurchaseHistory(models.Model):
    _name = 'total.purchase.history'

    order_id = fields.Many2one('purchase.order', string='Purchase Order', ondelete='cascade')
    purchase_order_id = fields.Many2one('purchase.order', string='Purchase Order', ondelete='cascade')
    date_order = fields.Datetime(string="Order Date")
    partner_id = fields.Many2one('res.partner', related='purchase_order_id.partner_id', string="Customer", store=True)
    company_id = fields.Many2one(related='order_id.company_id', store=True, index=True)
    ...
    sequence = fields.Integer(string="Sequence",
                              help="Gives the sequence order when displaying a list of purchase quote lines.", default=10)
    ...
    product_uom_qty = fields.Float(string='Quantity', required=True, digits='Product Unit of Measure', default=1)
    price_unit = fields.Monetary(string="Unit price")
    taxes_id = fields.Many2many('account.tax', string="Taxes", store=True, context={'active_test': False},
                              check_company=True)
    price_tax = fields.Monetary(string="Tax Amount")
    discount = fields.Float(string="Disc%")
    discount_amount = fields.Monetary(string="Discount")
    ...
    def _compute_amount_in_pkr(self):
        for line in self:
            if line.currency_id.name != 'PKR':
                rate = line.env['res.currency.rate'].search([('currency_id', '=', line.currency_id.id), ('name', '=', date.today())], limit=1)
                if rate:
                    print(f"Currency: {line.currency_id.name}, Date: {date.today()}, Rate: {rate.inverse_company_rate}")
                    line.amount_in_pkr = line.price_total * rate.inverse_company_rate  # Convert to PKR
                    line.conversion_rate_pkr = rate.inverse_company_rate  # Currency to PKR
...
class ProductPurchasePartsHistory(models.Model):
    _name = 'purchase.product.parts.history'
